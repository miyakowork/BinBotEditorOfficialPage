...
describe('Faq', () => {
  it('reveals one answer through an accessible disclosure button', async () => {
    ...
    expect(screen.getByText('v0.10.1 适用于搭载 Apple Silicon 的 Mac；AI 翻译需要用户自行配置受支持服务商的 API Key。')).toBeVisible()
    ...
    expect(
      screen.getByText(
        '核心编辑、格式化、时间、cURL 解析、编码与加密工具无需云端；HTTP 请求与 AI 翻译只会在用户主动发送时联网。',
      ),
    ).toBeVisible()
  })
})
