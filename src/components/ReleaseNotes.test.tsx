...
describe('ReleaseNotes', () => {
  it('keeps the latest release visible and compacts the complete history behind native disclosures', async () => {
    ...
    expect(within(entries[0]).getByRole('heading', { name: 'v0.10.2' })).toBeVisible()
    expect(within(entries[0]).getByText(/CodeMirror 字符宽度/)).toBeVisible()
    expect(within(entries[0]).getByText(/编辑内容与行号区/)).toBeVisible()
    expect(within(entries[0]).getByText(/透明度合成动画/)).toBeVisible()

    const archive = screen.getByText('展开 10 个历史版本')
