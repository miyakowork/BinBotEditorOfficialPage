...
describe('site content contract', () => {
  ...
  it('keeps release history newest first and highlights the current release', () => {
    expect(releaseHistory.map(({ version }) => version)).toEqual(['v0.10.2', 'v0.10.1', 'v0.10.0', 'v0.9.0', 'v0.8.0', 'v0.7.2', 'v0.7.1', 'v0.7.0', 'v0.6.1', 'v0.6.0', 'v0.5.0'])
    expect(releaseHistory[0]).toMatchObject({ version: 'v0.10.2', latest: true })
    ...
    expect(releaseHistory[0].highlights).toEqual(expect.arrayContaining([
      ...
      expect.stringContaining('字体参数'),
      expect.stringContaining('透明度'),
    ]))
    expect(releaseHistory[7].highlights).toEqual(expect.arrayContaining([
